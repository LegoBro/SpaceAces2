...
execute as @e[type=minecraft:item_frame] at @s run function tick:item_frame/tick
...
execute as @e[type=minecraft:armor_stand,tag=healing_machine_display] at @s unless entity @e[type=minecraft:slime,distance=..1,tag=healing_machine] run kill @s
...
execute as @e[type=minecraft:area_effect_cloud,tag=energy_orb] at @s run function classes:shocksmith/energy_sap/follow
execute as @e[type=minecraft:armor_stand,tag=over_heal_machine] at @s run function classes:healer/over_heal_machine/standby

# kill aoe's summoned by creepers
kill @e[type=minecraft:area_effect_cloud,nbt={effects:[{id:"minecraft:resistance"}]}]
...
kill @e[type=!minecraft:player,x=3,y=195,z=-10,dx=11,dy=1,dz=15]
