...
execute if entity @s[scores={crouching=0}] run tp @e[tag=cannon,sort=nearest,limit=1] ~ ~1.6 ~ ~ ~
execute if entity @s[scores={crouching=1..}] run tp @e[tag=cannon,sort=nearest,limit=1] ~ ~1.26 ~ ~ ~
execute store result score @e[tag=cannon,sort=nearest,limit=1] id run scoreboard players get @s id
execute as @e[tag=cannon,sort=nearest,limit=1] at @s run function classes:gunner/projectile
