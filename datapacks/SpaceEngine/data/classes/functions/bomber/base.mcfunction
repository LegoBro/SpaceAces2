item replace entity @s[tag=!invis,scores={crouching=0},gamemode=!spectator] armor.head with minecraft:leather_helmet{HideFlags:63,AttributeModifiers:[{AttributeName:"generic.movement_speed",Name:"generic.movement_speed",Amount:0.05,Operation:2,UUID:[I;-801381860,1374964116,-1510517210,267107606]}],Unbreakable:1,display:{color:2593195},Enchantments:[{id:"minecraft:binding_curse",lvl:1}]}
item replace entity @s[tag=!invis,scores={crouching=1..}] armor.head with minecraft:leather_helmet{HideFlags:63,AttributeModifiers:[{AttributeName:"generic.movement_speed",Name:"generic.movement_speed",Amount:-0.1,Operation:2,UUID:[I;-801381860,1374964116,-1510517210,267107606]}],Unbreakable:1,display:{color:2593195},Enchantments:[{id:"minecraft:binding_curse",lvl:1}]}

execute if entity @s[scores={activate_second=0}] run item replace entity @s hotbar.1 with carrot_on_a_stick{display:{Name:"{\"translate\":\"sticky_bomb\",\"color\":\"white\"}",Lore:["{\"translate\":\"sticky_bomb_lore1\",\"color\":\"white\"}","{\"translate\":\"sticky_bomb_lore2\",\"color\":\"white\"}","{\"translate\":\"sticky_bomb_lore3\",\"color\":\"white\"}"]},HideFlags:63,CustomModelData:11040001,Unbreakable:1}
execute if entity @s[scores={activate_second=1..}] run item replace entity @s hotbar.1 with gray_dye
scoreboard players set @s[nbt={SelectedItemSlot:1},scores={rightClick=1..}] activate_second 60
scoreboard players reset @s[nbt={SelectedItemSlot:1},scores={rightClick=1..}] rightClick
execute if entity @s[scores={activate_second=60}] run function classes:bomber/sticky_bomb/activate
scoreboard players remove @s[scores={activate_second=1..}] activate_second 1
execute if entity @s[scores={activate_second=55}] run function classes:bomber/sticky_bomb/deactivate

execute if entity @s[scores={activate_third=0}] run item replace entity @s hotbar.2 with carrot_on_a_stick{display:{Name:"{\"translate\":\"detonator\",\"color\":\"white\"}",Lore:["{\"translate\":\"detonator_lore1\",\"color\":\"white\"}","{\"translate\":\"detonator_lore2\",\"color\":\"white\"}"]},HideFlags:63,CustomModelData:11040002,Unbreakable:1}
execute if entity @s[scores={activate_third=1..}] run item replace entity @s hotbar.2 with gray_dye
scoreboard players set @s[nbt={SelectedItemSlot:2},scores={rightClick=1..}] activate_third 60
scoreboard players reset @s[nbt={SelectedItemSlot:2},scores={rightClick=1..}] rightClick
execute if entity @s[scores={activate_third=60}] run function classes:bomber/detonator/activate
scoreboard players remove @s[scores={activate_third=1..}] activate_third 1
execute if entity @s[scores={activate_third=40}] run function classes:bomber/detonator/deactivate

# Passive
item replace entity @s hotbar.3 with scute{display:{Name:"{\"translate\":\"bomb_voyage\",\"color\":\"white\"}",Lore:["{\"translate\":\"bomb_voyage_lore1\",\"color\":\"white\"}","{\"translate\":\"bomb_voyage_lore2\",\"color\":\"white\"}"]},HideFlags:63,CustomModelData:11040003,Unbreakable:1}
execute if entity @s[scores={health=..0}] unless entity @s[team=red,tag=escape] run function classes:bomber/death_impact

scoreboard players set @s maxHealth 150

item replace entity @s[scores={reload=0,shoot=0}] hotbar.0 with carrot_on_a_stick{display:{Name:"{\"translate\":\"plasma_grenade_launcher\"}",Lore:["{\"translate\":\"plasma_grenade_launcher_lore\",\"color\":\"white\"}","{\"text\":\" - damage: 10\",\"color\":\"white\",\"italic\":false}","{\"text\":\" - ammo: 8\",\"color\":\"white\",\"italic\":false}","{\"text\":\" - reload with speed: 2.5s\",\"color\":\"white\",\"italic\":false}"]},HideFlags:63,CustomModelData:11040000,Unbreakable:1}
item replace entity @s[scores={reload=0,shoot=1..}] hotbar.0 with paper{display:{Name:"{\"translate\":\"plasma_grenade_launcher\"}",Lore:["{\"translate\":\"plasma_grenade_launcher_lore\",\"color\":\"white\"}","{\"text\":\" - damage: 10\",\"color\":\"white\",\"italic\":false}","{\"text\":\" - ammo: 8\",\"color\":\"white\",\"italic\":false}","{\"text\":\" - reload with speed: 2.5s\",\"color\":\"white\",\"italic\":false}"]},HideFlags:63,CustomModelData:11040000}
execute if score @s[nbt={SelectedItemSlot:0},scores={shoot=0}] rightClick matches 1.. run scoreboard players set @s shoot 1
execute if score @s[nbt={SelectedItemSlot:0},scores={shoot=1}] rightClick matches 1.. run scoreboard players remove @s totalShots 1
scoreboard players reset @s[nbt={SelectedItemSlot:0}] rightClick
execute if score @s shoot matches 1 run function classes:bomber/shoot
scoreboard players add @s[scores={shoot=1..}] shoot 1
scoreboard players set @s[scores={shoot=25..}] shoot 0
scoreboard players add @s[scores={totalShots=..0,reload=0}] reload 1
execute if score @s reload matches 1.. run function classes:bomber/reload
scoreboard players set @s[scores={crouching=1..}] crouching 0
