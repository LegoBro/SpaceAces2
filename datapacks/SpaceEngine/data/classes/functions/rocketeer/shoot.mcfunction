...
summon armor_stand ~ ~ ~ {Tags:["rocket","raycast","grenade"],DisabledSlots:4144959,Invisible:1b,NoBasePlate:1b,Marker:1b,Small:1b,Pose:{Head:[180f,0f,0f]}}
...
execute if entity @s[scores={crouching=0}] run tp @e[tag=rocket,limit=1,sort=nearest] ~ ~.6 ~ ~ ~0
execute if entity @s[scores={crouching=1..}] run tp @e[tag=rocket,limit=1,sort=nearest] ~ ~.26 ~ ~ ~0
execute store result score @e[tag=rocket,limit=1,sort=nearest] id run scoreboard players get @s id
execute as @e[tag=rocket,limit=1,sort=nearest] at @s positioned ~ ~1.25 ~ run function classes:rocketeer/projectile
execute as @s at @s run playsound gun.rocket hostile @a ~ ~ ~