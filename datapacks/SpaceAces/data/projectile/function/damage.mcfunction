...
tag @n[type=!minecraft:item_frame,tag=!ignore,tag=!ignore.projectiles,tag=!raycast,sort=nearest,limit=1,distance=..3,tag=!projectile.piercing.ignore] add projectile.target
...
execute as @e[type=#projectile:players,tag=hb] if score @s id = place_id id at @s run function projectile:hit_indicator
