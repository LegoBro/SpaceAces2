...
execute as @a[scores={health=..0},tag=in_game,team=blue] run scoreboard players remove blue_lives Numbers 1
execute as @a[scores={health=..0},tag=in_game,team=red] run scoreboard players remove red_lives Numbers 1

gamemode spectator @a[tag=in_game,team=spectator]
tag @a[tag=in_game,team=spectator] remove in_game
execute as @a[tag=in_game,scores={health=..0}] run tp @s @e[type=item_frame,tag=general_spawn,limit=1,sort=random]
execute at @a[scores={health=..0},tag=in_game] run playsound minecraft:item.totem.use player @a ~ ~ ~ 50 2 1
...
execute if score blue_lives Numbers matches 0 run bossbar set minecraft:blue name ["",{"text":"IGSR Players Left","color":"blue"}]
execute if score red_lives Numbers matches 0 run bossbar set minecraft:red name ["",{"text":"RLU Players Left","color":"red"}]
