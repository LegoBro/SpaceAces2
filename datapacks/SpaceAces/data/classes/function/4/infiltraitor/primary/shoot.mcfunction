...
execute anchored eyes run tp @e[type=minecraft:area_effect_cloud,tag=rifle,tag=new,sort=nearest,limit=1,distance=..3] ^ ^ ^ ~ ~
execute as @e[type=minecraft:area_effect_cloud,tag=rifle,tag=new,sort=nearest,limit=1,distance=..3] at @s run function classes:4/infiltraitor/primary/projectile
