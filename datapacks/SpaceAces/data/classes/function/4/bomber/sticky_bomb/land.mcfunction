...
summon item_display ^ ^ ^-0.5 {Tags:["entity","standby_sticky_bomb","hb_player","class.deployable","headshot_fix","new"],teleport_duration:1}
item replace entity @n[type=minecraft:item_display,tag=standby_sticky_bomb,tag=new,tag=entity,distance=..3] container.0 with minecraft:paper[item_model="bomber/sticky"]
