...
execute anchored eyes run tp @e[type=minecraft:area_effect_cloud,tag=cannon,sort=nearest,limit=1,distance=..3] ^ ^ ^ ~ ~
...
execute as @e[type=minecraft:area_effect_cloud,tag=cannon,sort=nearest,limit=1,distance=..3] at @s run function classes:3.4/gunner/projectile
