...
execute anchored eyes run tp @e[type=minecraft:area_effect_cloud,tag=swipe,sort=nearest,limit=1,distance=..1.5] ^ ^ ^ ~ ~
...
execute as @e[type=minecraft:area_effect_cloud,tag=swipe,sort=nearest,limit=1,distance=..1.5] at @s positioned ~ ~-1 ~ run function classes:brawler/projectile
