...
execute as @e[type=slime,tag=turret_base,distance=..500] if score @s id = place_id id run scoreboard players operation turretHealth Numbers = @s health
execute unless score turretHealth Numbers = xp Numbers run function classes:mechanic/last_resort/reload
