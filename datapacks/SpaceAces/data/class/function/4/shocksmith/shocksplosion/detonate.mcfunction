...
execute as @n[type=#projectile:players,tag=sab.activePlayer,tag=attacker] facing entity @e[distance=1..50,tag=!ignore,tag=!ignore.projectiles] feet positioned ^ ^ ^ run function projectile:boomer/create
...
execute as @e[type=#projectile:players,tag=sab.activePlayer] if score @s Team = #Team Team run function class:4/shocksmith/shocksplosion/heal
scoreboard players set @n[type=#projectile:players,tag=sab.activePlayer,tag=attacker] ultimate_charge 0
