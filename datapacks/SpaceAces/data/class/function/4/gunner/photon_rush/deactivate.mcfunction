...
execute facing entity @e[distance=1..6,tag=hb,tag=!is_on_team] feet positioned ^ ^ ^ run function projectile:boomer/create
execute as @e[distance=..0.999,tag=hb,tag=!is_on_team] unless score @s Team = #team Team run function class:4/bomber/sticky_bomb/damage_near
