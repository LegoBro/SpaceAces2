...
execute as @e[type=#projectile:players,tag=hb] if score @s id = place_id id run tag @s add target.owner
execute as @n[type=#projectile:players,tag=hb,tag=target.owner] unless score @s Class matches 9 run kill @e[distance=..1,tag=turret_base,sort=nearest,limit=1]
...
execute if entity @s[tag=sniper] as @e[tag=!ignore.projectiles,tag=!invis,distance=..60,tag=!ignore,tag=hb] unless score @s Team = Team Team run tag @s add target.enemy
execute if entity @s[tag=rocket] as @e[tag=!ignore.projectiles,tag=!invis,distance=5..45,tag=!ignore,tag=hb] unless score @s Team = Team Team run tag @s add target.enemy
...
execute as @e[tag=target.enemy] if score @s uuid.0 = target target.0 if score @s uuid.1 = target target.1 if score @s uuid.2 = target target.2 if score @s uuid.3 = target target.3 run function class:4/helper/los/check
