...
execute as @e[type=#projectile:players,tag=hb] if score @s id = place_id id run scoreboard players add @s totalShots 1
execute as @e[type=#projectile:players,tag=hb,scores={reload=1..}] if score @s id = place_id id run scoreboard players add @s reload 20