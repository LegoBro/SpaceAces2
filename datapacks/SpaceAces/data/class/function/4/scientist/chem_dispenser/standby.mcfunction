...
execute as @e[type=#projectile:players,tag=sab.activePlayer] if score @s id = #target_owner Numbers run tag @s add target.owner
...
tag @e[type=#projectile:players,tag=sab.activePlayer] remove class.target
return 1