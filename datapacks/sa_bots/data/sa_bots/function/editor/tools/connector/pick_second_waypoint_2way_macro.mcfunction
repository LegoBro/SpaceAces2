...
$execute as b-0-0-0-$(waypoint_source_string) store result score #subroute_source sab.var if entity @s[tag=sab.botSubroute]


#----------
#target: add source to target's incoming waypoint list
...
#----------
#source: add target to source's outgoing waypoint list
...
#----------
#target: add source to target's outgoing waypoint list
...
scoreboard players operation #compare sab.var = #waypoint_source sab.var
data modify storage sa_bots:waypoint list set value "outgoing"
execute if data entity @s data.outgoing[0] run function sa_bots:editor/tools/connector/test_for_duplicate_ids with storage sa_bots:waypoint

#successful add to incoming list on target
$execute if score #duplicate_found sab.var matches 0 run data modify entity @s data.outgoing prepend value [0,"$(waypoint_source_string)",[0],[-1]]
execute if score #duplicate_found sab.var matches 0 store result entity @s data.outgoing[0][0] int 1 run scoreboard players get #waypoint_source sab.var
#if source is a subroute, set subroute reference to 0 (meaning we need to define this, later)
execute if score #duplicate_found sab.var matches 0 if score #subroute_source sab.var matches 1 run data modify entity @s[tag=!sab.botSubroute] data.outgoing[0][3][0] set value 0
execute if score #duplicate_found sab.var matches 0 if score #subroute_source sab.var matches 1 run tag @s[tag=!sab.botSubroute] add sab.hasUndefinedEndpoints
#----------


#----------
#source: add target to source's incoming waypoint list
...
scoreboard players operation #compare sab.var = #waypoint_target sab.var
data modify storage sa_bots:waypoint list set value "incoming"
$execute as b-0-0-0-$(waypoint_source_string) if data entity @s data.incoming[0] run function sa_bots:editor/tools/connector/test_for_duplicate_ids with storage sa_bots:waypoint

#successful add to outgoing list on source
$execute if score #duplicate_found sab.var matches 0 as b-0-0-0-$(waypoint_source_string) run data modify entity @s data.incoming prepend value [0,"$(waypoint_target_string)",0,0,0]
$execute if score #duplicate_found sab.var matches 0 as b-0-0-0-$(waypoint_source_string) store result entity @s data.incoming[0][0] int 1 run scoreboard players get #waypoint_target sab.var
#----------


#tell player if subroutes are undefined
execute if entity @s[tag=sab.hasUndefinedEndpoints] run return run tellraw @a[tag=sab.botPathEdit] ["\n",{translate:"sa_bot.prompt.set_endpoint_for_sub_route",color:white,italic:true}]
